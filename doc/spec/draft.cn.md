...
在期望实际开发中，`.abcdml`文档主要是充当隐式源文件的作用，文件中的ABCDML内容主要用作开发的中间数据和存储数据，开发者真正面对的是集成开发工具的可视化界面。这是出于对开发者的非程序员身份的考量。如非必要，开发者不需要真正的去编辑ABCDML。但一般而言，了解A4L的语法和思想，有助于开发、调试你的A4L应用程序。同时，完全用ABCDML编写A4L也是可行的，只是这需要强大的图形想象力。

## 1. Scoop

这份标准草案定义了A4L的词法、语法和模块包的开发与使用，并描述了ABCDML抽象语法应当呈现出的布局与交互。
...
我们将从一个最简单的组件开始，通过一点点增加更复杂的语法内容，来最终定义一个相对复杂的组件。

### 5.1. 命名规范

任何组件都必须有一个组件名，组件名在一个项目里面是唯一的。
组件名使用大坨峰命名风格，下面是一个名为HelloWorld的组件：
...
这个组件是一个宽度为100%父级宽度，高度微100%父级高度的白色背景组件, 组件上有黑色`Hello, this is my first A4L App.`字样。

组件名前可以添加命名空间，这将使得复杂应用中可以出现同名组件，如下所示带有明明空间的组件便是：

```abcdml
# src/components/HelloWorld.abcdml
...
namespace/HelloWorld:
  ...
  []: __
```
*此处`__`表示画板中没有图层，它还有几个相似意义，本案将在对应章节进行定义*
