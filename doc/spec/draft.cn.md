...
空值被定义为用来表示组件的属性值为空，在类型申明中用`empty`表示。

`empty`有两个别名，分别为`void`和`null`，他们所表示的意义完全一样。设计别名的意义是为了在必要时增加代码的可读性，如果因为使用了别名导致代码混乱难读则失去了设计别名的初衷，所以如果不能对别名所代表的意义进行区隔和严格规范，那么别名将不被建议使用。本案示例将统一使用empty，如使用别名，必定额外说明缘由。

空值一般不单独申明，往往和其他类型并列申明，表示该值可被赋予值，也可以不赋值或者赋空值`EMPTY`。
