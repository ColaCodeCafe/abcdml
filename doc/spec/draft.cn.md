...
辅助函数的定义和实现不属于ABCDML的描述范畴，但申明回调函数和调用辅助函数计算是ABCDML语法所支持的。我们希望辅助函数的定义一致，在不同的实现下表现出的效果一致，故将辅助函数的定义一同规范于在本案之后的辅助函数章节中。

### 2.2. Development

开发A4L主要就是合理的设计组件并将他们组织起来，而组织组件的形式其实是设计更高层的组件，归根结底设计A4L程序就是设计A4L的各层组件，也就是编写ABCDML文档。

任何人可以使用任何文字编辑软件来编写ABCDML。

不过，由于A4L主要面向于非程序员，更准确的说是面向UI设计师，所以A4L在设计之初就将可视化集成开发工具考虑进来，从词法和语法设计上就为集成工具的实现作了准备。这些包括但不限于方便开发工具做检查的标量子类型系统和便于开发工具作调试功能的监控模块。这些特性是运行环境可以忽略的特性，完全是设计用来开发和调试的。本案相关章节中提及的调试方案皆是适用于生产环境的调试方案，关于使用监控模块进行调试的介绍将在附录中说明。
...
A4L是申明式的程序设计语言。语言本身不具备直接执行的能力，可编译到其他语言执行，也可由其他语言来解释执行。A4L的实现准则如下：

1. 编译执行或解释执行的起点是ABCDML抽象语法树（ABCDML AST），实现的第一步骤便是根据ABCDML和FND生成ABCDML抽象语法树。关于抽象语法树的定义，由本案进行规范，不得独自设计。
2. 用于实现的语言及其下用于实现ABCDML的框架必须支持本案中所提到的所有A4L特性，如果目标框架（编译执行）或运行时环境（解释执行）不能达到此要求，则不能用来实现完整的标准A4L。
3. 编译执行或解释执行都需要先将ABCDML抽象语法树中的函数事先编译为一个或多个函数库, 保证输入输出一致。内置标准函数可以提前编译并集成到目标框架中。完整版本的A4L实现必须对辅助函数进行支持，且必须保证与本文档的描述的表现一致。
4. 编译执行需要将ABCDML抽象语法树中组件转译成目标框架中外观、交互等一致的组件。解释执行需要提供一个运行时环境，使其能根据ABCDML抽象语法树动态创建外观、交互等一致的组件。实现者亦可混合处理。

理论上，解释执行拥有更强的可调试性，编译执行则拥有更高的性能。不过，这取决于实现者自己的设计和拿捏，本案只关注A4L的实现结果，不干涉A4L的实现细节。
...
这一章将定义ABCDML语法的主要构成，这些语法结构u用于定义你的组件主体。
